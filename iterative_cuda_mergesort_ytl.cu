...
/*
 * How much shared memory is used for each threadblock for the portion of program
 * which sorts exclusively using shared memory.
 */
#define TILE_SIZE_SHARED_MEM_SORT THREADS_PER_BLOCK_SHARED_MEM_SORT * 8
/*
 * Number of threads per threadblock used for portion of program which sorts using
 * both global and shared memory.
 */
#define THREADS_PER_BLOCK_GLOBAL_MEM_SORT THREADS_PER_BLOCK_SHARED_MEM_SORT / 4
/*
 * Amount of shared memory per threadblock used as linear caching buffer in portion
 * of program which sorts using both global and shared memory
 */
 ...
 */
/*
 * I genuinely forgot which GitHub repo where I got this following useful macro from
 * but not my original work :/
 */
#define CHECK(call) { \
	cudaError_t err; \
...
int main() {
    ...
    while (fgets_return_stat == NULL) {
      ...
      if (fgets_return_stat != NULL) {
         ...
         errno = 0;
         input_array_length = strtoul(user_input, &remaining_non_ulong, USER_INPUT_NUM_BASE);
         int strtol_error_status = errno;

         if (strtol_error_status != 0 || remaining_non_ulong[0] != '\0' || input_array_length < 1) {

             fgets_return_stat = NULL;

             printf("Error: invalid input - please press enter again to bring back up\n"
    ...
    printf("Time took to sort array of length %ld on GPU: %lf seconds.\n",
                input_array_length, difftime(end_func_call.tv_sec, start_func_call.tv_sec) +
                                                (((double) (end_func_call.tv_nsec -
                                                                start_func_call.tv_nsec)) / NUM_NS_PER_SEC));
