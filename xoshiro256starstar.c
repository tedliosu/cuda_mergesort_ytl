...
// Obtained using wget https://prng.di.unimi.it/xoshiro256starstar.c
...
#define ARRAY_S_LEN 4

static inline uint64_t rotl(const uint64_t x, int k) {
  return (x << k) | (x >> (64 - k));
}

static uint64_t s[ARRAY_S_LEN];

void init_xoshiro256starstar(void) {
  for (int index = 0; index < ARRAY_S_LEN; ++index) {
    s[index] = splitmix64_get_next();
  }
}
